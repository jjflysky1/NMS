...
namespace EventRead
{
    class mail
    {
        public void Event_sendmail(string Eventtext)
        {
            
            DBCON.Class1 DBCON = new DBCON.Class1();
            MySqlConnection CON = new MySqlConnection(DBCON.DBCON);
            string SQL = "";
            SQL = "select distinct a.*, b.mailip from  event_mail_target a , mail_info b where eventname = '" + Eventtext.ToString() + "'";
            ...
            foreach (DataRow row in DBSET.Tables["BD"].Rows)
            {
                ...
                try
                {
                    ...
                    using (HttpClient client = new HttpClient())
                    {
                        ...
                        formData.Add(new StringContent(Eventtext + " 이벤트가 발견 되었습니다"), "msg");                // 메세지 내용
