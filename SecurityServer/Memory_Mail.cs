    ...
    class Memory_Mail
    {
        public void memory_sendmail(string serverip, string nowmemroy)
        {
            try
            {
                ...
                SQL = "select distinct   a.email, a.mailno, c.phone , c.mailip, c.memorylimit , mail_sender from mail_target a , service b ,  mail_info c where c.flag=1 and  DATE_ADD(b.memorymail, INTERVAL 10 MINUTE) < now() and a.serverip = b.ServerIP" +
                ...
                foreach (DataRow row in DBSET.Tables["BD"].Rows)
                {
                    ...
                    //        formData.Add(new StringContent(serverip + " 서버 현재 메모리 알림 기준치인 " + row["memorylimit"].ToString() + " 넘어 " + nowmemroy + " 입니다."), "msg");                // 메세지 내용
                    ...
                    if (row["email"].ToString() != "")
                    {
                        try
                        {
                            ...
                            MAIL.Subject = "메모리 알림";
                            MAIL.Body = serverip + " 장비 현재 메모리 알림 기준치인 " + row["memorylimit"].ToString() + " 넘어 " + nowmemroy + " 입니다. ";
            ...
            {
                Console.WriteLine("메일 안 에러 : " + e.Message);
            }
        }
