...
namespace SecurityServer
{
    class Cpu_Mail
    {
        public void cpu_sendmail(string serverip, string nowcpu)
        {
            ...
            string SQL = "";
            SQL = "select distinct   a.email, a.mailno, c.phone , c.mailip, c.cpulimit , mail_sender from mail_target a , service b ,  mail_info c where c.flag=1 and  DATE_ADD(b.cpumail, INTERVAL 10 MINUTE) < now() and a.serverip = b.ServerIP" +
            ...
            foreach (DataRow row in DBSET.Tables["BD"].Rows)
            {
                ...
                if (row["email"].ToString() != "")
                {
                    try
                    {
                        ...
                        MAIL.Subject = "Cpu 알림";
                        MAIL.Body = serverip + " 장비 현재 Cpu 알림 기준치인 " + row["cpulimit"].ToString() + " 넘어 " + nowcpu + " 입니다. ";
